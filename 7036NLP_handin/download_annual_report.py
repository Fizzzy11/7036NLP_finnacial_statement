...
download_Progress = readTxt('Progress of Downloading Annual Report Information.txt')  

if not os.path.exists("annual report"):
    os.mkdir('annual report')
...
def download(rows, lock, num):
    # Reads the data line by line, returning a dictionary for each line
    for row in rows:
        tem = row['id'].split(':')
        ...
        if res.status_code == 200:
            ...
            file_name = 'annual report/' + row['title'] + '-' + row['year'] + '-' + row["cik"] + '.html'            
...
if __name__ == '__main__':
    lock = threading.Lock()  # Using thread locks
    with concurrent.futures.ThreadPoolExecutor(max_workers=cpu_count) as executor:
        futures = []
        # Downloading with Thread Pools
