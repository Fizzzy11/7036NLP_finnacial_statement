...
if not os.path.exists("data.csv"):
        with open('data.csv', mode='a', newline='',encoding='utf8') as file:
            ...
            writer.writerow(['标题','截止时间','cik','id'])   
...
download_Progress = readTxt('年报信息下载进度.txt')# 读取已下载年报信息下载进度

url = 'https://efts.sec.gov/LATEST/search-index'
params = {
    #'from':'100',data_json['hits']['total']["value"]此值代表数据数量，超过100条则需要翻页，可通过‘数据数量除100的倍数是否大于等于1判断是否需要翻页’
    'dateRange': 'all',
    'category': 'custom',
    'entityName': '0000927971',
    'forms': '10-K',#如果只需要10-K，删除20-F，字母必选大写
    'startdt': '2019-01-01',
    'enddt': '2025-02-03'
}
...
def get_cik():
    ...
    for cik,title in zip(result['cik'],result['证券简称']):
        cik_ = str(cik)
...
for full_cik,cik,title in zip(full_ciks,ciks,titls):
    ...
    with open('年报信息下载进度.txt', mode='a', newline='') as file:
        file.write(cik + '\n')
        print(f'{cik}完成')
